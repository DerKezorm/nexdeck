...
- **NetBird.** The peers of a NetBird network, self-hosted or on NetBird Cloud, read with a personal access token; a service user is the tidy choice. Two cards: **Peers**, one line each with its NetBird address, its system and when it was last seen, and **Status** with how many are connected. A peer whose login has expired is shown as such and first, not just as away: it stays cut off until somebody signs it in again, and nothing on its side says so. **Beta:** the refusals were recorded from a live netbird-server 0.79.0, the peers follow NetBird's published API description. Asked for in issue #13.
