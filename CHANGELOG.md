...
- **ReadMeABook.** The audiobook request and download engine, read through an administrator's API token. Three cards: an **overview** with what is downloading, all requests, what finished and failed in the last 30 days and the users, coloured by ReadMeABook's own health and with its reasons, such as downloads stuck for more than a day; **downloads** with how far each book is, the speed and the time left; and the **latest requests** with who asked and where each stands, a failed one in red with ReadMeABook's reason, optionally only the open ones. A new request and a book that reached the library are told as "a new media request" and "a download finished". A user's token is turned away by ReadMeABook from the figures the cards read, and the connection test says so and whose token it is. Needs ReadMeABook 1.2.0 or newer. Beta: seen against ReadMeABook 1.2.3, but a running download was never seen. Requested in issue #19.
...
- **The dot of an app tile says why it is the colour it is.** Hovering it shows the answer time, the last error, "Not checked yet", or that the check is switched off. A grey dot used to say nothing at all.
- **YouTube subscriptions.** A card with the newest videos of the channels an account follows, for those who want their subscriptions rather than a hand-made list. YouTube hands no home feed to any API, so the card reads the account's subscriptions, the most active first, once a day through the Data API with a key on the YouTube connection, and the videos from the same free channel feeds as the Videos card, several at a time. The key travels in a request header, not in the address, so it never reaches the log. A handle is looked up on the channel page and costs no quota. Every refusal says what to do: subscriptions set to private, the daily quota used up, the API not switched on for the key's project, or a key YouTube does not take.
- **This machine.** A card for the machine nexdeck runs on, read from what the Linux kernel writes anyway: processor share between two looks, memory, swap, load, the warmest temperature sensor, uptime, and the file system nexdeck keeps its data on, which in Docker is a volume of the host. No agent and nothing to mount: measured in a container limited to 256 MB and one processor, /proc showed the host's memory, processors and uptime, because the limits live in the container's cgroup. Hovering a row shows what does not fit beside its bar, such as "5.7 GB of 7.6 GB".
- **One list of updates.** A card that gathers what has a newer version: the containers What's Up Docker and Cup list, Watchtower when its last run could not update everything (it updates by itself, so a run that worked is nothing to do), the newest release of up to ten GitHub projects, one from the last seven days standing out, and nexdeck itself. nexdeck is asked about only through the update check in System: with that switched off, the card shows what a check by hand found and sends nothing out. A source that fails becomes a line above the rows, and the others still show.
- **Status page.** A card that lists every card with a reachability check, on its own board or on every board its owner has: how each answered last and how fast, its row of availability bars and the share of the time it was up, what is down at the top. The share is rounded down, so 100 per cent means every check answered. It can show only what is down, and draw the availability of all services side by side as bars. Boards of other people never appear on it, because the card is read once for everyone who sees it.
- **Notices on the board.** The notice centre as a card, for a wall where nobody opens the bell: newest first, filtered by level or to the unread ones, with the number of unread ones on top. It shows the notices of the board's owner to everyone who can see the board, and says so in its description; it never marks one as read.
...
- **A card without a check starts a fresh one.** The settings sheet stays in the background between cards and kept the check settings of the card opened before it: switching the check on for a card that had none took over the other card's kind, interval and TLS box. The same is why switching the check off and on again did not help in issue #17.
...
- **NetBird.** The peers of a NetBird network, self-hosted or on NetBird Cloud, read with a personal access token. Two cards: **Peers**, one line each with its NetBird address, its system and when it was last seen, and **Status** with how many are connected. A peer whose login has expired is shown as such and first, not just as away: it stays cut off until somebody signs it in again, and nothing on its side says so. A service user with the plain User role is enough: it reads every peer, also with "regular users view blocked" on, as a fresh installation starts. Both cards ran against a live netbird-server 0.79.0 with two clients, so NetBird starts without the beta badge. Asked for in issue #13.
- **Pangolin.** The sites of the reverse proxy and what it serves from them, through Pangolin's Integration API, self-hosted or on Pangolin Cloud. Three cards: **Status** with sites online and offline, public and private resources and how many are unhealthy; **Sites**, online, offline, local or waiting for approval; and **Resources**, public ones with their address and health, private ones with their destination and offline when every site that carries them is. The key needs three permissions, one for each list, and a key missing one is told which, in Pangolin's own words. A self-hosted Pangolin runs the Integration API only with `enable_integration_api` in its config.yml, on port 3003, and the connection sheet says so. **Beta:** built from Pangolin's source (1.23), not yet run against a live one. Asked for in issue #14.
